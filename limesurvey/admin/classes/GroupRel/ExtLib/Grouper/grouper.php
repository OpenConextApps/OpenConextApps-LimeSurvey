 ...
 * Grouper Client
 * 
 * Provided by SURFnet bv (www.surfnet.nl)
 * 
...
class Grouper
{
	...
	try {
//		print_r($args); // print_r($this->_client->getMembersLite($args)->return);
    ...
    public function getSubjects($id, $gid) {
		$args = array(
			'clientVersion' => 'v1_5_000', // self::clientVersion,
			'subjectId' => 'urn:collab:person:test.surfguest.nl:mdobrinic',		
//			'subjectId' => 'urn:collab:person:surfnet.nl:niels',
//			'sourceId' => 'urn:collab:person:surfnet.nl:niels',
			'groupName' => $gid,
			'subjectAttributeNames' => '',
			'memberFilter' => '',
			'actAsSubjectId' => $this->_uuid,
			'actAsSubjectSourceId' => '', // $id, // $gid, //'',
			'actAsSubjectIdentifier' => '', // $id, // '',
			'fieldName' => '',
			'includeGroupDetail' => 'false',
...
//print_r($args); exit();    	    	
    	try {
    		$result = $this->_client->getSubjectsLite($args)->return;
    		print_r($result);
    	} catch (Exception $e) {
    		error_log($e->getMessage());
    		return -1;
